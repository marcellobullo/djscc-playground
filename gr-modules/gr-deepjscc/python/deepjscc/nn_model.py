...
#
# Shared PyTorch model definitions for ADJSCC (Attention-based Deep JSCC).
# Pure PyTorch — no GNU Radio, no ZMQ, no I/O side effects.
# Import from here whenever you need the encoder/decoder networks.
...
# =====================================================================
class Args:
    """Minimal arg container for the networks (only `tcn` is currently used)."""
...
def load_pretrained_weights(module, checkpoint_path, prefix):
    ...
    into `module`. Strips the prefix before calling load_state_dict.
    Returns True on success, False otherwise."""
    if not os.path.exists(checkpoint_path):
        print(f"nn_model: Checkpoint not found: {checkpoint_path}")
        return False
    try:
        checkpoint = torch.load(checkpoint_path, map_location=torch.device('cpu'), weights_only=False)
        full_state = checkpoint["net"]
        filtered = {k.replace(prefix, ''): v for k, v in full_state.items() if k.startswith(prefix)}
        module.load_state_dict(filtered, strict=False)
        print(f"nn_model: Loaded {len(filtered)} weight tensors (prefix '{prefix}').")
        return True
    except Exception as e:
        print(f"nn_model ERROR: loading weights: {e}")
        return False
